...
- [~] **A5.6.5 Candidate scheduling polish** — The public booker is locked to a simpler month-first calendar and displays the selected time in the candidate's timezone. Booking confirmation, cancellation, rescheduling, calendar-add links, and provider logos are consistent across Google Meet, Microsoft Teams, and Zoom. Event templates now offer a branded Share & embed flow with copy-paste inline, floating-button, and custom-button snippets. A candidate-owned Playwright regression covers timezone-aware booking, confirmation details, rescheduling, cancellation, and persisted state transitions. Remaining: validate the complete candidate journey on desktop and mobile with a real booking, including confirmation and reminder delivery.
