...
- [~] **R5.5.9 Secret and repository hygiene** — Audited tracked configuration files and removed a fixed cron API key from `.env.example`. A pre-commit check now blocks local `.env` files and recognizable live Stripe, Resend, Google OAuth, webhook, and private-key material from staged content. The repeatable `yarn security:scan-tracked` command scans all tracked source for provider-specific credentials without printing their values, while test-container secrets are now derived at CI runtime. Remaining release-blocking work: rotate previously shared credentials and confirm production secrets are injected by the deployment platform.
