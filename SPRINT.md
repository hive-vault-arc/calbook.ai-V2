...
- [~] **R5.5.3 Waitlist concurrency and recovery** — Waitlist joins use a database-enforced deterministic deduplication key. Promotion redemption uses an exact event/slot/tier/email claim with a 30-minute recovery lease, and only the winning claim can consume the single-use invitation. Promotion delivery retries three times without issuing a different token. A credential-protected five-minute cron now replaces each expired invitation with at most one successor for the exact slot and tier in a single transaction; active redemption leases are preserved, recovery is concurrency-safe, and the previous unsafe broad queue scan was removed. Twenty-seven focused service and endpoint tests pass. Migration `20260829173500_add_waitlist_redemption_claims` is applied to the development database. Remaining: add cancellation-path concurrency coverage and surface the existing cancellation-tier limitation wherever it affects user expectations.
...
- [~] Focused unit tests for billing, payment idempotency, entitlement boundaries, and waitlist concurrency. Waitlist concurrency/retry/recovery coverage passes (27 tests); payment acceptance remains deferred to the final pre-launch gate.
