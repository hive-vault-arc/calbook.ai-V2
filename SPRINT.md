...
- [~] **R5.5.8 Product/documentation alignment** — App-store metadata for Google Calendar and Google Meet identifies CalBook.ai and no longer links users to Cal.com. The app-store presentation layer now treats legacy Cal.com/Cal.diy publisher records as CalBook.ai-managed, routing their public links and support contact to the configured CalBook.ai deployment while preserving genuine third-party publisher details. Brand defaults now use CalBook.ai rather than Cal.com/Cal.diy, and absent support configuration no longer sends customers to a Cal.com address. Remaining: audit and replace residual customer-visible Cal references outside the app store, publish Terms and Privacy pages with final business details, set a monitored support mailbox, and complete a final customer-surface audit before launch. Verify branding and trademark assumptions before public launch.
