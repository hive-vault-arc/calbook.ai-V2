...
- [~] **R5.5.3 Waitlist concurrency and recovery** — Waitlist joins now use a database-enforced deterministic deduplication key. Promotion redemption uses an exact event/slot/tier/email claim with a 30-minute recovery lease, and only the winning claim can consume the single-use invitation. Promotion delivery retries three times without issuing a different token; 22 focused tests cover duplicate joins, concurrent claims, claim release/consumption, expired or consumed reuse, and email failure/retry behavior. Migration `20260829173500_add_waitlist_redemption_claims` is applied to the development database. Remaining: wire scheduled recovery for abandoned promotions, add cancellation-path concurrency coverage, and surface the existing cancellation-tier limitation wherever it affects user expectations.
...
- [~] Focused unit tests for billing, payment idempotency, entitlement boundaries, and waitlist concurrency. Waitlist concurrency/retry coverage passes (22 tests); payment acceptance remains deferred to the final pre-launch gate.
