...
- [~] **R5.5.6 Staging acceptance journeys** — Vercel Hobby now serves the full-stack Next.js application with Neon: the landing, login, signup, and health routes return successfully, and health verifies database and SMTP connectivity. The release branch's shared constants are Edge-compatible so the social image route can build on Vercel without loading `node:process`. Stripe/platform billing and distributed Redis remain intentionally degraded until their final configuration gates. Render's free 512 MB service still accepts connections without returning application responses and is not a viable runtime for this monolith. Remaining: run authenticated organization onboarding, booking, waitlist, cancellation, email, calendar, and final deferred payment journeys on Vercel and record failures as release blockers.
- **Staging finding:** the seeded local `pro@example.com` account is not present in Neon, so authenticated acceptance requires a dedicated staging account. The release preview correctly serves same-domain Terms and Privacy links and CalBook branding; the public production alias still tracks `main`, not the V1 release branch.
- [~] **R5.5.7 Legal, support, and incident readiness** — Added a support/incident runbook with escalation roles, severity targets, rollback authority, workflow guides, and customer communication templates. Public, same-domain Terms and Privacy pages now cover the scheduling service, connected providers, candidate data, open-source licensing, GDPR-style rights, and Google API Limited Use disclosures; production validation requires a monitored public support email. Remaining release-blocking work: assign named owners, configure the monitored support address, obtain legal review with the operator's final entity/jurisdiction details, and finalize the cancellation/refund policy.
- [~] **R5.5.8 Product/documentation alignment** — App-store metadata for Google Calendar and Google Meet identifies CalBook.ai and no longer links users to Cal.com. The app-store presentation layer treats legacy Cal.com/Cal.diy publisher records as CalBook.ai-managed, routes their public links and support contact to the configured CalBook.ai deployment, and normalizes legacy Cal platform names/URLs in app-detail descriptions while preserving vendor URLs and integration behavior. Brand defaults, public form/API titles, signup CTAs, embedded branding, payment setup copy, email-client search, and shared customer translations now use CalBook.ai; missing support/terms configuration no longer sends customers to a Cal.com address. A configurable CalBook.ai help-center layer now replaces Cal.com links in command search, embeds, booking guidance, API keys, blocklists, webhooks, and advanced event-type settings; it remains inside the app until a public help center is configured. Remaining: audit integration-specific setup copy, publish Terms and Privacy pages with final business details, set a monitored support mailbox and public help center, and complete a final customer-surface audit before launch. Verify branding and trademark assumptions before public launch.
- **Brand audit follow-up:** organization emails, push notifications, onboarding invite placeholders, profile guidance, 404 documentation, and the account-menu roadmap no longer fall back to Cal.com-owned addresses or copy. Support, help-center, and roadmap destinations are deployment-configured; unconfigured roadmap links stay hidden.
- [~] **R5.5.9 Secret and repository hygiene** — Audited tracked configuration files and removed a fixed cron API key from `.env.example`. Pre-commit and CI checks now block local environment files (including deployment-specific variants) and recognizable live Stripe, Resend, Google OAuth, Neon, Render deploy-hook, webhook, and private-key material. Both checks share one tested pattern catalog, and the repeatable `yarn security:scan-tracked` command scans tracked source without printing secret values; the tracked scan passes on the release branch. Test-container secrets are derived at CI runtime. Remaining release-blocking work: rotate previously shared credentials and confirm production secrets are injected by the deployment platform.
...
- [~] **R5.5.11 Transactional email and calendar-provider readiness** — Resend transport precedence, environment validation, and live health verification are complete. Local Gmail SMTP is verified reachable and a real booking (`muH8FxoojdeChzhZUiPSxf`) completed with an organizer, attendee, and added guest; organizer, attendee, and guest inbox delivery is confirmed. The calendar UI reports Google OAuth readiness and explains that calendar access is separate from email delivery. Google Calendar/Meet now has a production callback, consent-screen, and end-to-end release checklist, and the README no longer advises storing OAuth JSON in `.env`. OAuth configuration is now fail-closed: incomplete credentials do not show Google Calendar/Meet as installable and malformed JSON cannot crash auth initialization. A stale Google Meet configuration now rejects the booking before it writes data instead of silently switching the candidate to Cal Video. Remaining: configure the production OAuth consent screen with final public legal/support URLs and verify the Google Calendar event/Meet link end-to-end.
...
- [x] Full Vitest release baseline passes locally (429 files; 4,283 tests passed, 47 skipped, 3 todo) after aligning booking-package mocks, branded credits, dry-run booking shape, and cross-platform route discovery.
...
- [~] **A5.6.3 Simplify the interview-template workspace** — The template cards now present duration, interview format, host count, candidate link, and candidate-facing order as a compact brief; the full-card drag-and-drop interaction remains intact and is covered by a browser regression. Remaining: validate the list view with agency users and refine from their feedback.
- [x] **A5.6.4 Agency operating dashboard** — Replaced the generic home redirect with an authenticated dashboard built on the existing booking API: interviews today, upcoming interviews, unconfirmed interviews needing attention, and the next five scheduled interviews. It includes direct actions for templates, integrations, and availability, and is now the primary shell destination. No candidate persistence or ATS schema was introduced.
- [~] **A5.6.5 Candidate scheduling polish** — The public booker is locked to a simpler month-first calendar and displays an accessible date → time → details progress marker in both desktop and mobile layouts, with the selected time in the candidate's timezone. Booking confirmation, cancellation, rescheduling, calendar-add links, and provider logos are consistent across Google Meet, Microsoft Teams, and Zoom. Event templates now offer a branded Share & embed flow with copy-paste inline, floating-button, and custom-button snippets. Candidate-owned Playwright regression covers mobile guidance, timezone-aware booking, confirmation details, rescheduling, cancellation, and persisted state transitions. Remaining: validate the complete candidate journey on desktop and mobile with a real booking, including confirmation and reminder delivery.
- [~] **A5.6.6 ATS integration boundary** — The provider-neutral [ATS integration boundary](docs/architecture/ats-integration-boundary.md) defines context lookup and idempotent booking-outcome sync without adding an ATS or candidate schema to V1. Remaining: validate the first provider target with prospective agency customers before adding a Greenhouse, Lever, or similar connector.
