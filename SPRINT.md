...
- [~] **R5.5.6 Staging acceptance journeys** — The Docker build is deployment-safe, but Render's free 512 MB runtime stalls on the full Next.js application. Free prelaunch staging is moving to Vercel Hobby with Neon and Upstash; the active Vercel configuration omits unsupported frequent cron schedules while preserving the production schedule separately. Remaining: complete the Vercel deployment, configure its canonical URL and secrets, then run authentication, organization onboarding, booking, waitlist, cancellation, email, calendar, and final deferred payment journeys. Record failures as release blockers.
