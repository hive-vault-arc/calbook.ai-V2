...
async function handler(
  ...
  if (bookingRequiresPayment && booking && bookerEmail) {
    ...
    if (activePackage) {
      redeemedPackageId = activePackage.id;
    }
  }
  const hasPackageRedemption = redeemedPackageId !== null;

  if (!isConfirmedByDefault && noEmail !== true && !(bookingRequiresPayment && !hasPackageRedemption)) {
    tracingLogger.debug(
  ...
  if (bookingRequiresPayment && hasPackageRedemption && booking && redeemedPackageId) {
    tracingLogger.debug(`Booking ${organizerUser.username} redeeming package session ${redeemedPackageId}`);
    await bookingPackageService.redeemSession({
      packageId: redeemedPackageId,
      bookingId: booking.id,
    });
  } else if (bookingRequiresPayment) {
