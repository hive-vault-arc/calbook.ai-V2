...
const handlePayment = async ({
  ...
  const appData = apps?.[paymentAppCredentials.appId] as
    | { paymentOption?: string; currency?: string; price?: number; enabled?: boolean; depositAmount?: number }
    | undefined;
  ...
  const depositAmount = appData?.depositAmount
    ? convertToSmallestCurrencyUnit(appData.depositAmount, currency)
    : 0;
  ...
  } else if (paymentOption === "DEPOSIT" && depositAmount > 0) {
    paymentData = await paymentInstance.createDeposit(
      paymentPriceAndCurrency,
      depositAmount,
      booking.id,
      booking.userId,
