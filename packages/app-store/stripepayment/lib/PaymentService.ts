...
class StripePaymentService implements IAbstractPaymentService {
  ...
  async createTip(
    ...
    try {
      ...
      if (!paymentData) {
        throw new Error("Failed to create tip payment record");
      }
      return paymentData;
    } catch (error) {
      log.error("Stripe: Tip payment could not be created", bookingId, safeStringify(error));
      throw new Error("tip_payment_not_created_error");
    }
  }
  ...
  async createDeposit(
    ...
    try {
      if (paymentOptionEnum.parse(paymentOption) !== "DEPOSIT") {
        throw new Error("Payment option is not compatible with createDeposit method");
      }
      ...
      if (depositAmount <= 0 || depositAmount >= payment.amount) {
        throw new Error("Deposit amount must be greater than 0 and less than the total amount");
      }
      ...
      // Charge the deposit immediately via PaymentIntent
      const depositIntent = await this.stripe.paymentIntents.create(
        {
          ...
          metadata: {
            ...
            remainingAmount: (payment.amount - depositAmount).toString(),
            paymentType: "deposit",
          },
        },
        { stripeAccount: this.credentials.stripe_user_id }
      ...
      // Also create a SetupIntent to save the payment method for the remaining charge
      ...
      const paymentData = await prisma.payment.create({
        data: {
          ...
          amount: payment.amount,
          currency: payment.currency,
          externalId: depositIntent.id,
          data: {
            depositIntent,
            setupIntent,
            depositAmount,
            remainingAmount: payment.amount - depositAmount,
            stripe_publishable_key: this.credentials.stripe_publishable_key,
            stripeAccount: this.credentials.stripe_user_id,
          } as unknown as Prisma.InputJsonValue,
          fee: 0,
      ...
      log.error("Stripe: Deposit payment could not be created", bookingId, safeStringify(error));
      throw new Error("deposit_payment_not_created_error");
    }
  }

  async chargeRemaining(payment: Payment, bookingId: Booking["id"]): Promise<Payment> {
    try {
      if (!this.credentials) {
        throw new Error("Stripe credentials not found");
      }

      const paymentData = payment.data as unknown as StripePaymentData & {
        depositAmount?: number;
      ...
      if (!paymentData.setupIntent) {
        throw new Error("No setup intent found for deposit payment");
      }
      ...
      const setupIntent = paymentData.setupIntent;
      const customer = await this.stripe.customers.retrieve(setupIntent.customer as string, {
        stripeAccount: this.credentials.stripe_user_id,
      });
      if (!customer) {
      ...
      const remainingIntent = await this.stripe.paymentIntents.create(
        {
          ...
          metadata: {
            ...
            originalDepositPaymentId: payment.id.toString(),
          },
        },
        { stripeAccount: this.credentials.stripe_user_id }
      );
      ...
      log.error("Stripe: Could not charge remaining balance for payment", bookingId, safeStringify(error));
      throw new ErrorWithCode(ErrorCode.ChargeCardFailure, "could_not_charge_remaining_balance");
    }
  }
